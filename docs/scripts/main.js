...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (employerLoginForm) {
        employerLoginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch(API_BASE_URL + '/employers/login', {
                    method: 'POST',
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch(API_BASE_URL + `/candidates/profile/${candidateId}`);
                if (!res.ok) {
    ...
    if (progressBtn) {
        progressBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch(API_BASE_URL + `/candidates/progress/${candidateId}`);
                if (!res.ok) {
                    const error = await res.json();
    ...
    if (viewShortlistForm) {
        viewShortlistForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch(API_BASE_URL + `/employers/${employerId}/shortlist/${jobId}`);
                const data = await res.json();
                if (!res.ok) {
